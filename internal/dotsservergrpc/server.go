...
func (s *DotsServerGrpc) Exec(ctx context.Context, app *dotspb.App) (*dotspb.Result, error) {
	...
	// Generate input for DoTS app environment.
	dotsEnvInput := ""
	dotsEnvInput += strconv.Itoa(ourRank) + "\n"
	for _, inputFile := range inputFiles {
		dotsEnvInput += strconv.Itoa(int(inputFile.Fd()))
	}
	dotsEnvInput += "\n"
	for _, outputFile := range outputFiles {
		dotsEnvInput += strconv.Itoa(int(outputFile.Fd()))
	}
	dotsEnvInput += "\n"
	for i, socket := range sockets {
		...
		if socket == nil {
			...
			dotsEnvInput += strconv.Itoa(int(socket.Fd()))
		}
	}
	dotsEnvInput += "\n"
	dotsEnvInput += app.FuncName + "\n"
	...
	if err := cmd.Wait(); err != nil {
		execLog.WithFields(log.Fields{
			"err": err,
		}).Error("Error waiting on application")
		return nil, grpc.Errorf(codes.Internal, "Application error")
	}
